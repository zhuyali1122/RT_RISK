...
| **累计展期 (cumulative_extension)** | `SUM(disbursement_amount)` | 所有在 `raw_repayment` 中有 `repayment_type = 3`（展期结清）且 `repayment_date <= stat_date` 的 loan，其 `raw_loan.disbursement_amount` 之和 |
...
按放款月份（cohort）分组，计算各 cohort 的逾期表现。**仅显示 2026-02 及以后的放款月。**
...
按到期月（maturity_month）汇总。**仅显示 2026-02 及以后的到期月。**
